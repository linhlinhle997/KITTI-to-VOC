...
class VOCIngestor(Ingestor):
    ...
    def _get_image_detection(self, root, image_id):
        ...
        if not os.path.isfile(annotation_path):
            raise Exception(
                f"Expected annotation file {annotation_path} to exist.")
        ...
        if segmented:
            ...
            if not os.path.isfile(segmented_path):
                raise Exception(
                    f"Expected segmentation file {segmented_path} to exist.")
...
class VOCEgestor(Egestor):
    ...
    def egest(self, *, image_detections, root):
        ...
        for image_detection in image_detections:
            ...
            shutil.copyfile(
                image['path'], f"{images_path}/{image_id}.{src_extension}")
            ...
            if image['segmented_path'] is not None:
                ...
                shutil.copyfile(image['segmented_path'],
                                f"{segmentations_path}/{image_id}.png")
            ...
            ET.ElementTree(xml_root).write(
                f"{annotations_path}/{image_id}.xml")
